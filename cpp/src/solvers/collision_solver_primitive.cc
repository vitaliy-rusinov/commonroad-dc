...
int not_implemented_error(const CollisionObject &obj1,
		const CollisionObject &obj2, const char *solver_name) {
	std::cout
			<< std::string("collision function between object of types ")
					...
					+ std::string(solver_name) << std::endl;
	throw;
	return 0;
}
...
std::size_t collide_aabb_aabb(const CollisionObject &obj1,
	...
	res.set_result(
			primitive_queries::collisionDetection(
...
std::size_t collide_polygon_obj(const CollisionObject &obj1,
	...
	std::vector<TriangleConstPtr> mesh_triangles =
			(static_cast<const Polygon&>(obj1)).getTriangleMesh();
	for (auto &obj : mesh_triangles) {
		collide_bool_func_t func = matr_primitive.getSolverBoolFunction(
				OBJ_TYPE_TRIANGLE, obj2.getCollisionObjectType());
		if (!func) {
			return -1;
		}
...
std::size_t collide_polygon_polygon(const CollisionObject &obj1,
	...
	static solvers::PrimitiveSolver solver;
	static CollisionFunctionMatrix matr_primitive(&solver);
	std::vector<TriangleConstPtr> mesh_triangles_1 =
			(static_cast<const Polygon&>(obj1)).getTriangleMesh();
	std::vector<TriangleConstPtr> mesh_triangles_2 =
			(static_cast<const Polygon&>(obj2)).getTriangleMesh();
	for (auto &obj1 : mesh_triangles_1) {
		for (auto &obj2 : mesh_triangles_2) {
			collide_bool_func_t func = matr_primitive.getSolverBoolFunction(
					OBJ_TYPE_TRIANGLE, OBJ_TYPE_TRIANGLE);
			if (!func) {
				return -1;
			}
...
std::size_t collide_shape_group_obj(const CollisionObject &obj1,
	...
	static solvers::PrimitiveSolver solver;
	static CollisionFunctionMatrix matr_primitive(&solver);
	std::vector<ShapeConstPtr> sg_objects =
			(static_cast<const ShapeGroup&>(obj1)).unpack();
	for (auto &obj : sg_objects) {
		collide_bool_func_t func = matr_primitive.getSolverBoolFunction(
				obj->getCollisionObjectType(), obj2.getCollisionObjectType());
		if (!func) {
			return -1;
		}
		func(*obj, obj2, res, req);
...
std::size_t collide_shape_group_shape_group(const CollisionObject &obj1,
	...
	static solvers::PrimitiveSolver solver;
	static CollisionFunctionMatrix matr_primitive(&solver);
	std::vector<ShapeConstPtr> sg_objects_1 =
			(static_cast<const ShapeGroup&>(obj1)).unpack();
	std::vector<ShapeConstPtr> sg_objects_2 =
	...
	for (auto &obj_1 : sg_objects_1) {
		for (auto &obj_2 : sg_objects_2) {
			...
			collide_bool_func_t func = matr_primitive.getSolverBoolFunction(
					obj_1->getCollisionObjectType(),
					obj_2->getCollisionObjectType());
			if (!func) {
				return -1;
			}
...
std::size_t collide_tvobst_obj(const CollisionObject &obj1,
	...
	for (int cc1 = time_idx_start; cc1 <= time_idx_end; cc1++) {
		...
		collide_bool_func_t func = matr_primitive.getSolverBoolFunction(
				cur_obj->getCollisionObjectType(),
...
std::size_t collide_tvobst_tvobst(const CollisionObject &obj1,
	...
	int time_idx_start = std::min(tvobst1.time_start_idx(),
			tvobst2.time_start_idx());
	...
	for (int cc1 = time_idx_start; cc1 <= time_idx_end; cc1++) {
		...
		collide_bool_func_t func = matr_primitive.getSolverBoolFunction(
				obst_1->getCollisionObjectType(),
				obst_2->getCollisionObjectType());
		if (!func) {
			return -1;
		}
