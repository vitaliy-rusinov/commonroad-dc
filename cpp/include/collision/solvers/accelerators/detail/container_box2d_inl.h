...
class ContainerBox2D {
 public:
  ...
  ContainerBox2D(aligned_vector<collision::CollisionObject*> objects_in,
    ...
    broadphase_manager_ = new b2DynamicTree();
  ...
  inline int checkCollision(const CollisionObject* obj,
    ...
    if (req.enable_verification) {
      aligned_vector<int> collision_candidates2;
      {
        for (int i = 0; i < objects_optimized_id_.size(); i++) {
          // ground truth collisions between AABBs including border collisions
  ...
  inline int numchecks() const { return numchecks_; }

  inline int numcands() const { return numcands_; }

 private:
  b2DynamicTree* broadphase_manager_;

  std::vector<AABB> aabbs_;
  std::vector<b2AABB> box2d_objects_;
    ...
    for (auto el : objects_) {
      ...
      collision::RectangleAABBConstPtr rect(new collision::RectangleAABB(
          ...
          Eigen::Vector2d((curAABB->x_max + curAABB->x_min) / 2,
                          (curAABB->y_max + curAABB->y_min) / 2)));
      box2d_objects_.emplace_back();
      float eps = 1e-5;
      box2d_objects_.back().lowerBound =
          b2Vec2(curAABB->x_min - eps, curAABB->y_min - eps);
      box2d_objects_.back().upperBound =
          b2Vec2(curAABB->x_max + eps, curAABB->y_max + eps);
    }
  ...
  inline int getCandidates(ObjectProxy& proxy,
    ...
    b2AABB b2Obj;
    float eps = 1e-5;
    b2Obj.lowerBound = b2Vec2(curAABB.x_min - eps, curAABB.y_min - eps);
    b2Obj.upperBound = b2Vec2(curAABB.x_max + eps, curAABB.y_max + eps);
    aligned_vector<int> candidates2;
    ContainerCollisionRequestDataBox2D reqData(candidates2);
    broadphase_manager_->Query(&reqData, b2Obj);
    for (auto el : candidates2) {
      candidates.push_back(*((int*)(broadphase_manager_->GetUserData(el))));
    }
    return 0;
  }
